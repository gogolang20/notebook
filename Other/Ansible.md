...
https://note.youdao.com/ynoteshare/index.html?id=8c63ba60a1fe9493ff39e11278730dc7&type=note&_time=1649650055283)

```shell
yum install -y epel-release #都要装
yum install -y libselinux-python #都要装
yum install -y ansible #操作节点需要装
...
192.168.152.150
192.168.152.151
ansible sss -m command -a "hostname" -k -u root

[sss]
192.168.152.150 ansible_user=root ansible_ssh_pass=123456
192.168.152.151 ansible_user=root ansible_ssh_pass=123456
ansible sss -m command -a "hostname"

vim /root/mysh/ssh_send_key.sh
#!/bin/bash
rm -rf ~/.ssh/id/id_rsa*
...
for ip in 150 151
do
        sshpass -p$SSH_Pass ssh-copy-id -i $Key_Path "-o StrictHostKeyChecking=no" 192.168.152.$ip
done
sh /root/mysh/ssh_send_key.sh
ssh -o ' StrictHostKeyChecking=no' '192.168.152.150'
ssh -o ' StrictHostKeyChecking=no' '192.168.152.151'
ansible sss -m command -a "uname -a"
```
...
ansible-doc -s shell #具体用法
```

### command模块

```shell
ansible sss -m command -a "uptime" #sss可以用 * 表示设置的所有组,或者单个机器IP

ansible sss -m command -a "pwd"
ansible sss -m command -a "pwd chdir=/tmp" #先切换目录,在执行命令

ansible 192.168.152.150 -m command -a "pwd creates=/sss" #判断文件是否存在,存在则跳过,不存在则执行
[root@localhost ~]# ansible 192.168.152.150 -m command -a "pwd creates=/opt"
192.168.152.150 | SUCCESS | rc=0 >>
skipped, since /opt exists


ansible sss -a "ls /opt removes=/sss" #存在则执行,不存在则跳过
ansible sss -a "ls /opt removes=/opt"

ansible sss -m command -a "chmod 000 /etc/hosts warn=False" #执行命令,忽略告警信息
```
...
ansible-doc -s shell #linux查看命令参数

vim sss.txt & #从节点,后台运行一个vim进程
ansible sss -m shell -a "ps -ef | grep vim | grep -v grep"
```

批量执行脚本

```
需要执行的脚本,必须在客户端机器上存在,否则会报错文件不存在,shell模块的特点,因为还有一个专门执行脚本的script模块
```

### script模块

```
把管理机上的脚本远程的传输到被管理节点上去执行

vim /root/myscript/local_hostname.sh
pwd
hostname
chmod +x local_hostname.sh
ansible sss -m script -a "/root/myscript/local_hostname.sh" #指定本地脚本位置即可
...
- hosts: sss
  ...
  - name: hello
    command: hostname
ansible-playbook hello.yml #运行
ansible-playbook -C hello.yml #检查,没有实际执行
```

```shell
对.yml文件加密
[root@localhost mysh]# ansible-vault encrypt hello.yml 
New Vault password: 
Confirm New Vault password: 
Encryption successful
[root@localhost mysh]# cat hello.yml 
$ANSIBLE_VAULT;1.1;AES256
[root@localhost mysh]# ansible-vault view hello.yml 
Vault password: #输入加密的密码
[root@localhost mysh]# ansible-vault edit hello.yml 
Vault password: #输入加密的密码
[root@localhost mysh]# ansible-vault rekey hello.yml 
Vault password: #输入加密的密码
New Vault password: #输入新密码
Confirm New Vault password: 
Rekey successful
[root@localhost mysh]# ansible-vault decrypt hello.yml 
Vault password: 
